...
from utils.config import INPUT_TXT_DIR
from .schemas import RawDocument, DocumentMetadata

# --- Importations pour OCR (RapidOCR remplace EasyOCR) ---
try:
    import fitz  # PyMuPDF
    from PIL import Image
    from rapidocr_onnxruntime import RapidOCR

    logging.info("Initialisation du moteur RapidOCR...")
    ocr_engine = RapidOCR()
    logging.info("Moteur RapidOCR initialisé.")

except ImportError as e:
    logging.warning(f"Modules OCR (PyMuPDF, Pillow, rapidocr) non installés ou erreur: {e}. L'OCR pour PDF ne sera pas disponible.")
    fitz = None
    Image = None
    ocr_engine = None
...
def extract_text_from_pdf_with_ocr(file_path: str) -> Optional[str]:
    """Extrait le texte d'un fichier PDF en utilisant l'OCR (RapidOCR)."""
    if not fitz or not ocr_engine:
        logging.warning("Modules/Moteur OCR non disponibles. Impossible d'effectuer l'OCR.")
        return None
    ...
    try:
        ...
        for page_num in tqdm(range(len(doc)), desc=f"OCR de {os.path.basename(file_path)}"):
            ...
            pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))  # Résolution x2 pour l'OCR
            img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)

            try:
                img_np = np.array(img)
                result, _elapse = ocr_engine(img_np)
                # result est une liste de [box, text, score] (ou None si rien détecté)
                page_text = "\n".join([r[1] for r in result]) if result else ""
                text_content.append(page_text)
            except Exception as ocr_e:
                logging.error(f"Erreur lors de l'OCR de la page {page_num + 1} de {file_path} avec RapidOCR: {ocr_e}")
                continue
...
def extract_text_from_pdf(file_path: str) -> Optional[str]:
    """Extrait le texte d'un fichier PDF, avec fallback OCR (RapidOCR) si peu de texte est trouvé."""
    try:
...
def load_and_parse_files(input_dir: str) -> List[RawDocument]:
    ...
    Retourne une liste de RawDocument validés par Pydantic (les documents invalides
    -texte vide, métadonnées incomplètes, etc.- sont loggés et ignorés).
    """
...
#                extracted_content = extract_text_from_excel(str(file_path))            # fichier excel géré par load_excel_to_db
            ...
            # Nettoyage du point de l'extension (.pdf -> pdf)
            clean_ext = ext.lstrip(".")
            stem = Path(file_path).stem
            output_filename = f"{stem}_{clean_ext}.txt"
            output_path = Path(INPUT_TXT_DIR) / output_filename

            # Si c'est un dictionnaire (plusieurs feuilles Excel), un RawDocument par feuille
            if isinstance(extracted_content, dict):
                text_to_save=""
                for sheet_name, text in extracted_content.items():
                    documents.append(_build_raw_document(
                        text=text,
                ...
                output_path.write_text(text_to_save, encoding="utf-8")
                ...
                output_path.write_text(str(extracted_content), encoding="utf-8")
        
    # On filtre les None issus des échecs de validation Pydantic
    documents = [d for d in documents if d is not None]
    logging.info(f"{len(documents)} documents chargés, parsés et validés (Pydantic).")
    return documents


def _build_raw_document(
    text: str, source: str, filename: str, category: str, full_path: str, sheet: Optional[str] = None
) -> Optional[RawDocument]:
    """Construit et valide un RawDocument. Retourne None (+ log) si la validation échoue."""
    try:
        return RawDocument(
            page_content=text,
        ...
        logging.warning(f"Document invalide ignoré ({source}): {e}")
        return None
